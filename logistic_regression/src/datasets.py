...
def dataset_info(dataset: Dataset) -> str:
    ...
    im_shape = dataset[0][0].shape if len(dataset) > 0 else None
    im_dtype = dataset[0][0].dtype if len(dataset) > 0 else None
    return (
...
def dataloader_info(dataloader: DataLoader):
    ...
    batch_shape = next(iter(dataloader))[0].shape if len(dataloader) > 0 else None
    return (
...
def get_dataloader(dataset_name: str, batch_size: int, train: bool, **kwargs) -> DataLoader:
    if dataset_name == "mnist":
        ...
        ds = torchvision.datasets.MNIST(
            root=DATA_ROOT / "mnist", train=train, download=False, transform=transform
        )
    elif dataset_name == "cifar10":
        transform = Compose([ToTensor(), Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        ds = torchvision.datasets.CIFAR10(
            root=DATA_ROOT / "cifar10", train=train, download=False, transform=transform
        )
    else:
