...
COLOR_OPTIONS = {"red": 4, "yellow": 34, "green": 64, "cyan": 94, "blue": 124, "magenta": 154}
...
def otsu_threshold(counts: np.ndarray, bins: Optional[np.ndarray] = None) -> float:
    ...
    # plt.hist(counts, bins, color='blue', histtype="barstacked", edgecolor='black', align='left')
    # plt.xlabel('Counts')
    # plt.ylabel('Frequency')
    # # Drawing the line for the best threshold value which divides the datapoints into two clusters
...
def roundedness(moments: dict[str, float]) -> float:
    ...
    """
    m1 = np.array([moments['m10'], moments['m01']]) / moments['m00']
    m2 = np.array([[moments['m20'], moments['m11']], [moments['m11'], moments['m02']]])
    covariance = m2 / moments['m00'] - np.outer(m1, m1)
...
def threshold_on_hue(image: np.ndarray, color: str, hue_tolerance: int = 10) -> np.ndarray:
    ...
    if wrapped:
        ...
        binary_lower = cv.inRange(hsv, range_lo, (179, 255, 255))
        binary_upper = cv.inRange(hsv, (0, saturation_thresh, 0), range_hi)
        binary = cv.bitwise_or(binary_lower, binary_upper)
    else:
...
def is_shape_symmetric(
    ...
    """
    rot = cv.getRotationMatrix2D(centroid_xy, 180, 1)
...
def sample_points_on_ring(
    ...
    """
    locations_xy = [
        (center_xy[0] + radius * np.cos(theta), center_xy[1] + radius * np.sin(theta))
...
def identify_single_shape(binary: np.ndarray) -> str:
    ...
    if moments["m00"] == 0:
        # If the shape has zero area, it cannot be identified
        return "unknown"
    ...
    if roundedness(moments) < 0.5:
        ...
        if is_shape_symmetric(binary, centroid_xy, threshold=0.9):
            return "rectangle"
        else:
            return "wedge"
    else:
        ...
        samples = sample_points_on_ring(binary, centroid_xy, radius=1 * stdev, num_points=10)
        if np.all(samples):
            return "circle"
        else:
            return "cross"
...
def find_shapes(
    ...
    num_labels, labels, stats, centroids = cv.connectedComponentsWithStats(binary_this_color)
    ...
    hist_bins, hist_counts = np.histogram(areas, bins='auto')
    bin_centers = (hist_counts[1:] + hist_counts[:-1]) / 2
    area_threshold = otsu_threshold(hist_bins, bin_centers)
    if size == "small":
        return np.array(
            [
                loc
