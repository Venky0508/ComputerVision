...
def add_weights_channel(image: np.ndarray, border_size: int = 0) -> np.ndarray:
    ...
    weights_xy = weights_xy[:, :, np.newaxis]  # Add the third dimension for concatenation
...
def calculate_bounding_box_of_warps(homographies, images):
    ...
    """
    warped_corners = np.vstack(
        [
            apply_homography(
                h[:3][:3],
                np.array(
                    [[im.shape[0], im.shape[0]],
    ...
    left = int(np.round(np.min(warped_corners[0])))
    right = int(np.round(np.max(warped_corners[0])))
    top = int(np.round(np.min(warped_corners[1])))
    bottom = int(np.round(np.max(warped_corners[1])))
...
def stitch(
    ...
    if not all(len(p) == len(points[0]) for p in points):
        ...
        # so that we don't have to worry about overflow
    ...
    homographies = [
        cv.findHomography(points[j], points[reference_image_index])[0] for j in range(len(images))
    ]
    ...
    warped = [cv.warpPerspective(im, h, (new_h, new_w)) for h, im in zip(homographies, images)]
